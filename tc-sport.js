// tc-sport.js — Sports betting (pending until admin decides)
'use strict';
...
var BG_GRADIENTS=['linear-gradient(135deg,#1a3a7c,#e74c3c)','linear-gradient(135deg,#0d2460,#27ae60)','linear-gradient(135deg,#7c1a3a,#e67e22)','linear-gradient(135deg,#1a5c3a,#1a3a7c)','linear-gradient(135deg,#2d0a4a,#e74c3c)','linear-gradient(135deg,#0a3a2a,#2255b8)'];
...
function loadHome() {
  fbGet('/sportEvents').then(function(data){
    ...
    // Hide sport section on error
    var sl=$('sport-section-label');var sw=$('spscroll');
...
function renderSportScroll(events, emptyMsg) {
  var wrap=$('spscroll'); if(!wrap)return; wrap.innerHTML='';
  if(!events||!events.length){wrap.innerHTML='<div style="color:var(--txt2);padding:1rem;font-size:13px;">'+(emptyMsg||'No sports events.')+'</div>';return;}
  events.forEach(function(ev,idx){
    var card=document.createElement('div');card.className='spcard';
    var bg=BG_GRADIENTS[idx%BG_GRADIENTS.length];
    var odds=ev.odds||{h:1.85,d:3.20,a:4.10};
    var is2way=ev.type==='2way';
    var oddsHtml=is2way?
      '<div class="spod" data-sel="1" data-odds="'+odds.h+'"><div class="spod-n">'+((ev.team1||'').split(' ')[0])+'</div><div class="spod-v">'+Number(odds.h).toFixed(2)+'</div></div>'+
      '<div class="spod" data-sel="2" data-odds="'+odds.a+'"><div class="spod-n">'+((ev.team2||'').split(' ')[0])+'</div><div class="spod-v">'+Number(odds.a).toFixed(2)+'</div></div>':
      '<div class="spod" data-sel="1" data-odds="'+odds.h+'"><div class="spod-n">Home</div><div class="spod-v">'+Number(odds.h).toFixed(2)+'</div></div>'+
      '<div class="spod" data-sel="X" data-odds="'+(odds.d||3.2)+'"><div class="spod-n">Draw</div><div class="spod-v">'+Number(odds.d||3.2).toFixed(2)+'</div></div>'+
      '<div class="spod" data-sel="2" data-odds="'+odds.a+'"><div class="spod-n">Away</div><div class="spod-v">'+Number(odds.a).toFixed(2)+'</div></div>';
    card.innerHTML=
      ...
      '<div class="spbody">'+
        ...
        '<div class="splea">'+(ev.league||'League')+'</div>'+
        '<div class="spmatch">'+(ev.team1||'Team A')+' vs '+(ev.team2||'Team B')+'</div>'+
        '<div class="spodds">'+oddsHtml+'</div>'+
      '</div>';
    card.querySelectorAll('.spod').forEach(function(btn){
      btn.addEventListener('click',function(e){e.stopPropagation();openSportBet(ev,btn.dataset.sel,parseFloat(btn.dataset.odds));});
    });
...
function doSportSearch() {
  var q=($('ssinp').value||'').trim().toLowerCase();
  if(!q){renderSportScroll(allSportEvents);return;}
  var filtered=allSportEvents.filter(function(ev){
    return (ev.team1||'').toLowerCase().includes(q)||(ev.team2||'').toLowerCase().includes(q)||(ev.league||'').toLowerCase().includes(q)||(ev.sport||'').toLowerCase().includes(q);
  });
...
function openSportBet(ev,sel,odds) {
  ...
  markets.forEach(function(m){
    var d=document.createElement('div');d.className='bmkt'+(m.sel===sel?' sel':'');
    ...
    d.addEventListener('click',function(){
      ...
      mwrap.querySelectorAll('.bmkt').forEach(function(b){b.classList.remove('sel');});d.classList.add('sel');updateBsum();
    });
    mwrap.appendChild(d);
  });
  $('sbamt').value='';hd('sberr');updateBsum();st('sbbal',fmt(bal()));
  om('ov-sbet');
}

function updateBsum() {
  var amt=parseFloat($('sbamt').value)||0;
  ...
  // Win = bet * odds (payout only, not including bet)
...
function placeSportBet() {
  ...
  // Save as PENDING — admin will decide winner
  fbPush('/sportBets',{
    playerKey:CK,playerUid:CD.uid,playerName:CD.username,
    eventKey:ev.key||'',
    event:ev.team1+' vs '+ev.team2,
    league:ev.league,sport:ev.sport,
    selection:selName,selCode:sbState.sel,
    odds:sbState.odds,bet:amt,
    potentialPayout:potentialPayout,
    status:'pending',  // admin changes to 'won' or 'lost'
    settled:false,
    time:new Date().toISOString()
  }).then(function(){
    cm('ov-sbet');
    btn.textContent='Place Bet';btn.disabled=false;
    alert('Bet placed! Rs.'+amt.toLocaleString('en-NP')+' on '+selName+'.\nPotential win: '+fmt(potentialPayout)+'\n\nResult will be announced after the match. Your money is in the bet - if you win, full payout will be credited.');
    pushNotif('Bet placed: '+selName+' @ '+Number(sbState.odds).toFixed(2)+'x | Bet: Rs.'+amt.toLocaleString('en-NP')+' | Potential win: '+fmt(potentialPayout)+' | Waiting for result...','withdrawal');
  }).catch(function(e){
    // Refund if error
...
function loadHistory(type) {
  ...
  if(type==='sport'){
    fbGet('/sportBets').then(function(data){
      ...
      var bets=Object.entries(data).filter(function(e){return e[1].playerKey===CK;}).sort(function(a,b){return new Date(b[1].time)-new Date(a[1].time);});
        ...
        return '<div class="hitem">'+
          ...
          '<div style="text-align:right;flex-shrink:0;">'+
            ...
            (status==='won'?'<div style="font-size:10px;color:var(--txt2);margin-top:2px;">Bet: '+fmt(b.bet)+'</div>':
             ...
             '<div style="font-size:10px;color:var(--txt2);margin-top:2px;">Bet was held</div>')+
          '</div></div>';
      }).join('');
    ...
    fbGet('/playerTxns').then(function(data){
      ...
      var bets=Object.values(data).filter(function(b){return b.playerKey===CK;}).sort(function(a,b){return new Date(b.time)-new Date(a.time);}).slice(0,80);
